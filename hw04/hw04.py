...
def balanced(m):
    ...
    True
    """
    """if is_planet(m[1]) == True:
        return True
    if (length(left(m)) * total_mass(left(m))  == length(right(m)) * total_mass(right(m))):
        if is_mobile(end(left(m[1]))):
            if balanced(end(left(m))) == False:
                return False
        #`balanced(end(left(m))) and balanced(end(right(m)))`
        if is_mobile(end(right(m[1]))):
            if balanced(end(right(m))) == False:
                return False
        return True
    else:
        return False"""
    def torque(side):
        return length(side(m))*total_mass(end(side(m)))

    if torque(right) == torque(left):
        return (is_planet(end(left(m))) or balanced(end(left(m)))) and (is_planet(end(right(m))) or balanced(end(right(m))))
    else:
        return False
...
def max_path_sum(t):
    ...
    def secondary(t, sum):
        ...
        for branch in branches(t):
            #print (secondary(branch, sum + label(t)))
            max_branch = max(max_branch, secondary(branch, sum + label(t)))
        return max_branch
    return secondary(t, 0)
